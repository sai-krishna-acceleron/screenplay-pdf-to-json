...
from screenplay_pdf_to_json.parse_pdf import processInitialPages
...
def combineCategories(newScript, pageStart):
  ...
  for page in newScript:
    ...
    for i, content in enumerate(page["content"]):
      ...
      if not isinstance(content, dict) and "scene" not in content:
        continue
      ...
      while j < len(content["scene"]):
        ...
        if scene["type"] == "CHARACTER":
          ...
          sectionIsCharacter = lambda currScene, character: len(currScene["content"][character]) == 1 and isCharacter(
            currScene["content"][character][0])
          if sectionIsCharacter(scene, "character1") and sectionIsCharacter(scene, "character2"):
            ...
            last_scene_content = finalSections[-1]["content"][-1]["scene"][-1]["content"]
            if isinstance(last_scene_content, dict):
              if "character1" not in last_scene_content:
                last_scene_content["character1"] = {}
              last_scene_content["character1"]["dialogue"] = getJoinedText(scene["content"]["character1"])
              if "character2" not in last_scene_content:
                last_scene_content["character2"] = {}
              last_scene_content["character2"]["dialogue"] = getJoinedText(scene["content"]["character2"])
            elif isinstance(last_scene_content, list) and last_scene_content and isinstance(last_scene_content[0],
                                                                                            dict):
              if "character1" not in last_scene_content[0]:
                last_scene_content[0]["character1"] = {}
              last_scene_content[0]["character1"]["dialogue"] = getJoinedText(scene["content"]["character1"])
              if "character2" not in last_scene_content[0]:
                last_scene_content[0]["character2"] = {}
              last_scene_content[0]["character2"]["dialogue"] = getJoinedText(scene["content"]["character2"])
...
def categorizeSections(topTrends, script, pageStart, includeSceneNumber):
  ...
  for page in script:
    ...
    for i, content in enumerate(page["content"]):
      ...
      isTransition = content["segment"][0]["x"] >= 420 or "FADE" in text or (
          "CUT" in text and not isAction) or "TO:" in text
      ...
      if isHeading(content["segment"][0]):
        ...
        if i > 0 and len(finalSections[-1]["content"][-1]["scene"]) == 0 and y - page["content"][i - 1]["segment"][-1][
          "y"] < 24:
